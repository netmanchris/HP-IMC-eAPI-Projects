...
def dev_details(ip_address):
    if auth == None or url == None:  # checks to see if the imc credentials are already available
        imc_creds()
    global r
    get_dev_details_url = "/imcrs/plat/res/device?resPrivilegeFilter=false&ip="+ip_address+"&start=0&size=10&orderBy=id&desc=false&total=false"
...
def dev_vlans(devId):
    if auth == None or url == None:  # checks to see if the imc credentials are already available
        imc_creds()
    global r
    get_dev_vlans_url = "/imcrs/vlan?devId="+devId+"&start=0&size=10&total=false"
...
def dev_alarms(devId):
    if auth == None or url == None:  # checks to see if the imc credentials are already available
        imc_creds()
    global r
    get_dev_alarm_url = "/imcrs/fault/alarm?operatorName=admin&deviceId="+devId+"&desc=false"
